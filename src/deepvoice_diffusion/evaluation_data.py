...
def build_inventory(manifest_path: Path, external_dir: Path,
    ...
    for path, split in sorted(real.items()):
        inventory.append(dict(purpose="evaluation", file_id=_identity(Path(path)), path=path, source="real_manifest",
    ...
    for path in external:
        ...
        inventory.append(dict(purpose="evaluation", file_id=_identity(path), path=str(path), source="external",
